...
export const ProformaForm = () => {
  ...
  const fetchProformaInvoice = async () => {
    try {
      ...
      if (piData.items && piData.items.length > 0) {
        piData.items = piData.items.map(item => {
      ...
      setFormData(piData);
      // Fetch user who created this PI
      if (piData.created_by_user_id) {
        try {
          const usersRes = await api.getUsers();
          const creator = usersRes.data.find(u => u.user_id === piData.created_by_user_id);
  ...
  const handleConvert = async () => {
    ...
    try {
      if (convertTarget === 'quotation') {
        const res = await api.convertPIToQuotation(id);
        toast.success('Converted to Quotation');
        navigate(`/quotations/${res.data.quotation_id}`);
      } else if (convertTarget === 'soa') {
        const res = await api.convertPIToSOA(id);
        toast.success('Converted to SOA');
        navigate(`/soa/${res.data.soa_id}`);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await api.updateProformaInvoice(id, submitData);
        toast.success('Proforma Invoice updated');
      } else {
        await api.createProformaInvoice(submitData);
